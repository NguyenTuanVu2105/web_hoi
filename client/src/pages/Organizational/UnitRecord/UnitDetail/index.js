...
const UnitDetail = (props) => {
  ...
  const fetchData = async () => {
    setLoading(true);
    const result = await Unit.getUnitDetail(machihoi);
    setLoading(false);
  ...
  const handleUpdate = async (e) => {
    ...
    props.form.validateFields(async (err, values) => {
      if (!err) {
        setLoading(true);
        const { success } = await Unit.updateUnit(values);
        setLoading(false);
        if (success) {
      ...
      <div>
        <Form onSubmit={handleUpdate}>
            ...
            <Form.Item>
              <div style={{ width: "100%", display: "flex", flexWrap: "wrap" }}>
                <div className="reponsive-unit-s">
                  ...
                  <div>
                    <span className="label-unit-s">Tổng số thành viên:</span>
                    <br />
                  </div>
                </div>
          ...
          <div className="ButtonForMobileAdd">
            <Button
              className="buttonDisable1"
              id="roleedit"
              onClick={() => setchangeInput(false) && roles}
            >
              Sửa
            </Button>
            <Form.Item>
