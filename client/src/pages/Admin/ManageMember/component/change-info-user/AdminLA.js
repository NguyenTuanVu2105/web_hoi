...
function AdminLA(props) {
  ...
  const fetchData = async () => {
    setLoading(true);
    const result = await Admin.getLearnActivityAdmin(idUser);
    const result2 = await Admin.getActivityAdmin(idUser);
    setLoading(false);
  ...
  const handleSubmit = (e) => {
    ...
    props.form.validateFields(async (err, values) => {
      if (!err) {
        setLoading(true);
        const { success } = await Admin.editLearnActivityAdmin(values);
        setLoading(false);
        if (success) {
    ...
    <div className="para">
      <Form onSubmit={handleSubmit}>
        ...
        <div className="DivLAA">
          <Form.Item>
            <Button className="buttonLAA" type="primary" htmlType="submit">
              Submit
            </Button>
          </Form.Item>
        </div>
