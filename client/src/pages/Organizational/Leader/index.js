...
const Leader = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    const result = await Association.getLeaderAll();
    setLoading(false);
    if (result.success) {
      ...
      {leader.map((leader, index) => (
        <div key={"L" + index} className="para-team-leader">
          ...
          <div className="information1">
            <label className="label_information1">Họ tên: </label>
            <label
              style={{
                marginLeft: 2,
                fontSize: 14,
            ...
            <br />
            <label className="label_information1">Chức vụ: </label>
            <label
              style={{
                marginLeft: 2,
                fontSize: 14,
            ...
            >
              {leader.position.Chucvu}
            </label>
            <br />
            <label className="label_information1">Thời gian công tác: </label>
            <label
              style={{
                marginLeft: 2,
                fontSize: 14,
            ...
            <div className="div-reponsive-TL">
              <div
                id={"div-" + index}
                className="div-information-close"
                style={{ width: "100%", overflow: "hidden" }}
              >
                <label className="label_information1">
                  Thông tin chi tiết:{" "}
                </label>
                <p>{leader.Ghichukhac}</p>
              </div>
              <a
                id={"a-" + index}
                className="describe"
                onClick={() => HandleOC(index)}
              >
                {"Chi tiết >>>"}
              </a>
            </div>
