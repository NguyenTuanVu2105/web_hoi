...
const AddMemberModal = (props) => {
  ...
  const fetchDataPosition = async () => {
    const result = await Admin.getPosition();
    if (result.data.success) {
      setPosition(result.data.data);
    }
  };
  const fetchDataSpecialized = async () => {
    const result = await Admin.getSpecialized();
    if (result.data.success) {
  ...
  const handleSubmit = (e) => {
    ...
    props.form.validateFields(async (err, values) => {
      if (!err) {
        setLoading(true);
        const { success } = await Admin.addNewMember(values);
        setLoading(false);
        setVisible(false);
        if (success) {
        ...
        <Form.Item>
          {getFieldDecorator("gioitinh", {
            ...
            <Radio.Group name="radiogroup">
              ...
              <Radio value={0} class="radio_information">
                {" "}
                Nữ{" "}
              </Radio>
            </Radio.Group>
